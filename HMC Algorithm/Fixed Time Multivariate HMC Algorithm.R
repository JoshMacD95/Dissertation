...
#' Multivariate HMC Algortihm (Set Computation Time)
#'
...
HMC.fixedtime = function(x0, m, L, obs.time, 
  ...
  no.its = 0
  no.accept = 0
  draws = matrix(data = 0, nrow = 1, ncol = d + 2)
  
  draws[1,] = c(x.curr, U.curr, U.curr)
  
  while(Sys.time() - time.start < comp.time){
    # Momentum Step
    ...
    no.its = no.its + 1
    draws = rbind(draws, c(x.curr, U.curr, H.curr))
  }
  ...
  #
  # Important Output
  output(target, K, ESS, accept.rate, x0 = 0, no.its, L, obs.time)
  
  return(list(sample = draws[,1:d], log.target = -draws[,d+1], hamiltonian = draws[,d+2], 
              ESS = min(ESS), scaled.ESS = min(scaled.ESS), 
...
Test = HMC.fixedtime(x0, m, L, obs.time = stepsize*L, target, K, method = leapfrog, comp.time, burn.in)
